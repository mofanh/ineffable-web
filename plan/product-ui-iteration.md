...
- [ ] 收敛页面标题、指标、工具条、列表和运行记录的视觉层级。
- [ ] 保持创建、编辑、运行、删除、tick 和 Conversation 联动行为。
- [ ] 将英文错误与操作反馈统一为面向用户的中文产品文案。
- [ ] 检查桌面、窄内容区和右侧 Chat 展开时的容器响应。

自动验证：`npm run lint`、`npm run build`。

手动验证：空列表、搜索、创建、编辑、手动运行、查看会话、失败 run。

### Phase 2：账号与团队协作

- [ ] 账号页只突出真实资料与登录会话，移除或降级未接 API 的交互占位。

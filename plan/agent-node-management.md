...
- [x] Audit remediation：统一以 `conversation_id + workspace scope` 约束页面投影与 mutation，
  ...
  对话投影，输入框模式切换也发布同一失效信号；聊天侧所有投影请求使用目标身份与请求代际双重
  fencing，避免旧响应覆盖 mutation 后状态；移除非权威的前端推算版本号。
- [x] Final audit：完成基线到实现提交的独立只读审计；首轮与最终复审发现的跨页面同步、目标切换
  和同目标请求竞态均已集中修复。按照审计次数上限不再追加审计轮次，最终确定性门禁全部通过。

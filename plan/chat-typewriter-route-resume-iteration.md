...
- [x] 消除每个 SSE delta 的重复同步贴底，改由实际内容尺寸变化驱动。
- [x] 历史分页继续按条目数量变化恢复滚动位置。
- [x] 保留新消息、会话切换、用户上滑和 Thinking→正文过渡行为。
...
- [x] 缓存会话头部展示数据，避免每个 SSE delta 重建列表并触发分组计算。
- [x] 稳定刷新会话列表回调，减少聊天头部无意义的 prop 变化。
- [x] 缓存 Agent 文件选项分组，仅在筛选结果变化时重新计算。
...
- [x] 自动验证：`npm run lint`。
- [x] 自动验证：`npm run build`。
- [ ] 手动验证：会话搜索、切换、刷新及 `@` Agent 文件选择行为保持一致。

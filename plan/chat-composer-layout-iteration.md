...
Model 搜索面板支持从搜索框以 `ArrowDown` 进入第一个选项、从首项以
`ArrowUp` 返回搜索框；trigger 的可访问名称同时包含动作与当前模型。

## Phase 5：展开焦点精修

- [x] 点击展开 Model 或 Sandbox 时默认聚焦当前选项，不主动激活搜索输入框。
- [x] 搜索框继续支持鼠标进入、反向 Tab 与首项 `ArrowUp` 进入，不牺牲键盘可达性。
- [x] i18n、lint、build、Chat Runtime 与真实浏览器焦点验证通过。

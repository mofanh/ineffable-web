...
try {
  ...
  const openEditor = () => page.getByRole("button", { name: "Change password", exact: true }).click()
  ...
  assert.equal(await current.count(), 0, "password fields must not be exposed on the account overview")
  await openEditor()
  await current.waitFor()
  await current.fill("discard-this-password")
  await dialog.getByRole("button", { name: "Cancel", exact: true }).click()
  await dialog.waitFor({ state: "hidden" })
  await openEditor()
  assert.equal(await current.inputValue(), "", "cancel must clear sensitive input")
  ...
  await dialog.waitFor({ state: "hidden" })
  await openEditor()
  assert.equal(await dialog.getByRole("button", { name: /Resend in/ }).isDisabled(), true, "reopening must retain code cooldown")
  ...
  await dialog.waitFor({ state: "hidden" })
  for (const field of [current, next, confirm, code]) assert.equal(await field.count(), 0, "successful save closes the form")
  ...
  for (const path of ["password/code", "password/change"]) {
    ...
    if (path.endsWith("change")) {
      ...
      assert.equal(await dialog.isVisible(), true, "in-flight password change cannot be dismissed as cancelled")
    ...
    await dialog.waitFor({ state: "hidden" })
    await page.getByText("actor-b@example.com", { exact: true }).first().waitFor()
    await openEditor()

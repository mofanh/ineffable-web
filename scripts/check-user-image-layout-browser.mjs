...
try {
  ...
  for(const width of [1000,390,320]) {
    ...
    await page.getByRole("button",{name:"Native live image",exact:true}).click()
    const native=page.locator('[data-chat-row-key="native"]')
    await native.locator("img").waitFor()
    assert.equal(await native.locator("img").count(),1,"image-only native SSE output renders through ImageGallery")
    await native.getByRole("button",{name:"Image preview",exact:true}).click()
    await page.getByRole("dialog").waitFor()
    await page.keyboard.press("Escape")
    await page.getByRole("button",{name:"Native history image",exact:true}).click()
    await native.getByText("Edited image",{exact:true}).waitFor()
    await native.locator("img").waitFor()
    assert.equal(await native.locator("img").count(),1,"canonical history retains exactly one native image")

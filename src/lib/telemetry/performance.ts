...
export function observePerformance(emit: TelemetrySink) {
  let stopped = false
  const vital = (metric: { name: "INP" | "LCP" | "CLS"; value: number }) => {
    if (!stopped) emit({ name: "frontend_web_vital", properties: { metric: metric.name, value: metric.value } })
  }
  // Values only: never send attribution entries, DOM nodes or interaction text.
  onINP(vital); onLCP(vital); onCLS(vital)
  ...
  const tick = (now: number) => {
    frame = 0
    if (stopped || document.visibilityState !== "visible" || now > scrollUntil) { last = 0; return }
    if (last) {
      const gap = now - last
  ...
  return { flush, stop() {
    stopped = true; clearInterval(interval); observer?.disconnect(); cancelAnimationFrame(frame)

...
function Connections({ token, session }: { token: string; session: string }) {
  ...
  async function save() {
    ...
    try {
      const payload = { ...draft, allowed_private_ids: draft.allowed_private_ids.filter(Boolean), allowed_group_ids: draft.allowed_group_ids.filter(Boolean) }

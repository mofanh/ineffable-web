...
1. >= 1024px：Desktop 交互模式
   - 左右 Sidebar 可以常驻，左栏可折叠为 icon rail，右栏支持拖拽调整与全屏。
2. < 1024px：Compact 交互模式
   - 手机、平板与窄窗口统一使用该模式；左栏改为 Drawer，右栏使用全屏会话页。
3. `sm`、`md`、`lg`、`xl` 仍可用于页面内部的密度、列数与间距适配，但不得据此新增第三套设备交互模式。
